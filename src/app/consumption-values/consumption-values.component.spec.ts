...
describe('ConsumptionValuesComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      declarations: [ConsumptionValuesComponent, ControlValueFormFieldComponent],
  ...
  it('should update form input contents', async () => {
    ...
    for (const formControl of formControls) {
      const disabled = await formControl.isDisabled();

      // Form control should be initially valid (except disabled fields) and untouched
      expect(await formControl.isControlValid()).toBe(!disabled);
    ...
    for (const formControl of formControls) {
      ...
      // Form control should still be valid (except disabled fields) and untouched
      expect(await formControl.isControlValid()).toBe(!disabled);
      expect(await formControl.isControlTouched()).toBeFalse();
      expect(await formControl.isControlDirty()).toBeFalse();

      // Input field should contain the updated value (even disabled fields!)
      const input = await formControl.getControl(MatInputHarness);
      const value = await input.getValue();
